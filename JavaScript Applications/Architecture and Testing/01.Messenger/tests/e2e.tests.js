...
describe('tests preparation', function () {
	...
	describe('e2e tests', async () => {
		...
		it(`testing proper form submit`, async () => {
			...
			await page.route(
				'**/jsonstore/messenger*',
				request => request.fulfill(json({ author: 'Peter', content: 'Some text' }))
			)

			await page.fill('#author', 'Peter')
			await page.fill('#content', 'Some text')
			...
			expect(responseData).to.deep.eq({ 'author': 'Peter', 'content': 'Some text' })
		})
	})
})